...
import com.fresveg.supply.api.SupplyProblems;
...
public class FoundationConfiguration {
    @Bean
    java.time.Clock supplyClock() { return java.time.Clock.systemUTC(); }
    ...
    SecurityFilterChain foundationSecurity(HttpSecurity http, JsonMapper mapper,
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> {
                    ...
                    auth.requestMatchers(HttpMethod.GET, "/api/v1/supply/products/*/offers", "/api/v1/supply/listings/*", "/api/v1/supply/listings/*/availability").permitAll();
                    auth.requestMatchers(HttpMethod.GET, "/internal/v1/inventory/reservations/by-reference").authenticated();
                    auth.requestMatchers(HttpMethod.POST, "/internal/v1/inventory/order-reservations", "/internal/v1/inventory/reservations", "/internal/v1/inventory/reservations/*/commit", "/internal/v1/inventory/reservations/*/release").authenticated();
                    auth.requestMatchers("/api/v1/supply/**").authenticated().anyRequest().denyAll();
                })
                .build();
    }
    ...
    private static void unauthenticated(HttpServletRequest request, HttpServletResponse response, JsonMapper mapper)
            throws IOException {
        if (request.getRequestURI().startsWith("/api/v1/supply/") || request.getRequestURI().startsWith("/internal/v1/inventory/")) {
    ...
    private static void writeProblem(HttpServletRequest request, HttpServletResponse response, JsonMapper mapper,
        ...
        response.getOutputStream().write(mapper.writeValueAsBytes(SupplyProblems.create(status, code, detail, request)));
    }
}
