...
import com.fresveg.commerce.api.CommerceProblems;
...
public class FoundationConfiguration {
    @Bean
    java.time.Clock commerceClock() { return java.time.Clock.systemUTC(); }
    ...
    SecurityFilterChain foundationSecurity(HttpSecurity http, JsonMapper mapper,
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> {
                    ...
                    auth.requestMatchers("/api/v1/carts", "/api/v1/carts/**", "/api/v1/checkout/preview", "/api/v1/orders", "/api/v1/orders/**", "/api/v1/vendor/orders", "/api/v1/vendor/orders/**").authenticated().anyRequest().denyAll();
                })
                .build();
    }
    ...
    private static void unauthenticated(HttpServletRequest request, HttpServletResponse response, JsonMapper mapper)
            throws IOException {
        if (request.getRequestURI().equals("/api/v1/vendor/orders") || request.getRequestURI().startsWith("/api/v1/vendor/orders/") || request.getRequestURI().equals("/api/v1/orders") || request.getRequestURI().startsWith("/api/v1/orders/") || request.getRequestURI().equals("/api/v1/checkout/preview") || request.getRequestURI().equals("/api/v1/carts") || request.getRequestURI().startsWith("/api/v1/carts/")) {
    ...
    private static void writeProblem(HttpServletRequest request, HttpServletResponse response, JsonMapper mapper,
        ...
        response.getOutputStream().write(mapper.writeValueAsBytes(CommerceProblems.create(status, code, detail, request)));
    }
}
