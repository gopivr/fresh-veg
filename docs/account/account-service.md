...
Export `OIDC_ISSUER_URI`, `OIDC_AUDIENCE` and `OIDC_JWK_SET_URI`, along with the database settings in the [migration guide](../database/migration-guide.md). Docker Compose includes a [local Keycloak authorization server](../operations/local-authentication.md); signing keys are generated by the provider, not bundled in source. Use HTTPS issuer/JWK URLs in deployed environments; HTTP is supported for isolated local providers. The JWK endpoint has five-second connection/read timeouts. It is fetched when tokens need verification, without issuer discovery at startup; database health alone does not establish IdP availability.
