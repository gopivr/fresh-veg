...
import com.fresveg.fulfillment.api.FulfillmentProblems;
...
public class FoundationConfiguration {
    @Bean
    java.time.Clock fulfillmentClock() { return java.time.Clock.systemUTC(); }
    @Bean
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> {
                    auth.requestMatchers(HttpMethod.GET, "/actuator/health", "/actuator/health/liveness",
                            ...
                            .requestMatchers(HttpMethod.GET, "/api/v1/fulfillment/slots").permitAll();
                    if (docsEnabled) {
                        auth.requestMatchers(HttpMethod.GET, "/v3/api-docs", "/v3/api-docs.yaml").permitAll();
                    }
                    auth.requestMatchers(HttpMethod.GET, "/api/v1/fulfillments/*", "/api/v1/fulfillments/*/tracking").authenticated()
                            .requestMatchers(HttpMethod.POST, "/internal/v1/fulfillments", "/internal/v1/fulfillments/*/transitions", "/internal/v1/fulfillment/slots").authenticated()
                            .anyRequest().denyAll();
                })
                .build();
    }
    private static void unauthenticated(HttpServletRequest request, HttpServletResponse response, JsonMapper mapper) throws IOException {
        response.setHeader("WWW-Authenticate","Bearer");writeProblem(request,response,mapper,HttpStatus.UNAUTHORIZED,"SEC-401-001","A valid bearer token is required.");
    }
    ...
    private static void writeProblem(HttpServletRequest request,HttpServletResponse response,JsonMapper mapper,HttpStatus status,String code,String detail) throws IOException {
        response.setStatus(status.value());
        response.setContentType("application/problem+json");
        response.getOutputStream().write(mapper.writeValueAsBytes(FulfillmentProblems.create(status,code,detail,request)));
    }
}
