...
import com.fresveg.catalog.api.CatalogProblems;
...
public class FoundationConfiguration {
    ...
    SecurityFilterChain foundationSecurity(HttpSecurity http, JsonMapper mapper,
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> {
                    ...
                    auth.requestMatchers(HttpMethod.GET, "/api/v1/catalog/products", "/api/v1/catalog/products/*",
                            "/api/v1/catalog/categories", "/api/v1/catalog/categories/*/products").permitAll();
                    auth.requestMatchers("/api/v1/catalog/**").authenticated().anyRequest().denyAll();
                })
                .build();
    }
    ...
    private static void unauthenticated(HttpServletRequest request, HttpServletResponse response, JsonMapper mapper)
            throws IOException {
        if (request.getRequestURI().startsWith("/api/v1/catalog/")) {
    ...
    private static void writeProblem(HttpServletRequest request, HttpServletResponse response, JsonMapper mapper,
        ...
        response.getOutputStream().write(mapper.writeValueAsBytes(CatalogProblems.create(status, code, detail, request)));
    }
}
